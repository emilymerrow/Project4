...
const Post = require('../models/chore');
...
async function signup(req, res) {
  ...
  const filePath = `emiliamerrow/${uuidv4()}-${req.file.originalname}`;
...
async function profile(req, res){
  try {
    ...
    const chores = await Chore.find({user: user._id}).populate("user").exec();
    console.log(chores, ' this chores')
    res.status(200).json({chores: chores, user: user})
  } catch(err){
    console.log(err)
